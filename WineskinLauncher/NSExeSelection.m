...
{
    NSPortManager* port = [NSPortManager managerForPortAtPath:wrapperPath];
...
+(NSString*)getLNKDriveFromString:(NSString*)text
{
    ...
    NSString* nullHex = [NSString stringWithHexadecimalUTF8String:@"00"];
...
+(NSString*)getLNKPathFromString:(NSString*)text forType:(nonnull NSString*)type
{
    ...
    unichar char00 = [[NSString stringWithHexadecimalUTF8String:@"00"] characterAtIndex:0];
...
+(NSString*)getLNKFlagsFromString:(NSString*)text forPath:(NSString*)destino
{
    ...
    NSString* string10 = [NSString stringWithHexadecimalUTF8String:@"10"];
...
+(NSString*)getLNKFlagsFromRareString:(NSString*)text forPath:(NSString*)destino
{
    ...
    if (fragments.count <= 1) return nil;
    ...
    fragments = [text componentsSeparatedByString:[NSString stringWithHexadecimalUTF8String:@"00"]];
    
    // If the array has more than 1 item, it means that it has the path inside it
    if (fragments.count <= 1 || [fragments[1] length] <= 1) return nil;
    ...
    if ([part contains:subDestino]) return nil;

    return [part substringFromIndex:1];
}
...
+(NSString*)getLNKFlagsFromFileContents:(NSString*)text forPath:(NSString*)destino
{
    ...
    if (!pathFolder || pathFolder.length <= 2) return @"";
...
+(NSString*)getDESKTOPFlagsFromFileContents:(NSString*)text forPath:(NSString*)destino forPort:(NSString*)wrapperUrl
{
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:arquivo]) return @"";
    ...
    return arquivo ? [self getLNKFlagsFromFileContents:arquivo forPath:destino] : @"";
}
+(NSString*)getDESKTOPPathFromFileContents:(NSString*)arquivo forPort:(NSString*)wrapperUrl
{
    ...
    if (!temp)
    {
        ...
        temp = [self getEXEFileFromDESKTOPFileContents:arquivo];
        if (temp) return temp;
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:temp])
    {
        ...
        return temp ? [self getLNKPathFromFileContents:temp forType:@".EXE"] : @"";
    }
...
// INF files function
+(NSString*)getAutorunPathFromFile:(NSString*)arquivo
{
    @autoreleasepool
    {
        ...
        NSString *fileToOpen = [text getFragmentAfter:@"open=" andBefore:@"\n"];
        
        if (!fileToOpen || fileToOpen.length == 0)
        {
            fileToOpen = [text getFragmentAfter:@"OPEN=" andBefore:@"\n"];
        }
        ...
        NSString* CDFolder = [[arquivo stringByDeletingLastPathComponent] stringByAppendingString:@"/"];
        NSArray* arquivos = [NSPathUtilities filesCompatibleWithStructure:fileToOpen insideFolder:CDFolder];
        return arquivos.count == 0 ? nil : [arquivos firstObject];
    }
...
{
    NSString* macPath = [self getAutorunPathFromFile:arquivo];
    if (!macPath) return nil;
        
    return [NSPathUtilities getWindowsPathForMacPath:macPath ofWrapper:wrapperUrl];
}
...
+(NSString*)selectAsMainExe:(NSString*)arquivo forPort:(NSString*)wrapperUrl
{
    ...
    @autoreleasepool
    {
        ...
        if ([lowerCaseFile hasSuffix:@".lnk"])
        {
            ...
            if (text)
            {
                path  = [self getLNKPathFromFileContents:text forType:@".EXE"];
                flags = [self getLNKFlagsFromFileContents:text forPath:path];
            }
        }
        else if ([lowerCaseFile hasSuffix:@".desktop"])
        {
            ...
            if (text)
            {
                ...
                {
                    path  = [self getSteamExePathForPort:wrapperUrl];
                    flags = [self getURLFlagsFromSteamFileContents:text];
                }
                else
                {
                    path  = [self getDESKTOPPathFromFileContents:text forPort:wrapperUrl];
                    flags = [self getDESKTOPFlagsFromFileContents:text forPath:path forPort:wrapperUrl];
                }
            }
        }
        ...
        {
            path = [self getAutorunPathFromFile:arquivo forPort:wrapperUrl];
        }
        
        if (!path)
        {
            path = [self pathByUpdatingPath:arquivo forPort:wrapperUrl];
        }
        
        mainExePath = [NSString stringWithFormat:@"\"%@\" %@",
                       [path stringByReplacingOccurrencesOfString:@"/" withString:@"\\"],flags];
    }
